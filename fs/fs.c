...
static void partition_format(struct partition *part)
{
	...
	block_bitmap_sects =
		DIV_ROUND_UP(block_bitmap_bit_len, BITS_PER_SECTOR);
	...
	ide_write(hd, part->start_lba + boot_sector_sects, &sb,
		  super_block_sects);
	...
	buf_size = (buf_size >= sb.inode_table_sects ? buf_size :
	...
	uint32_t block_bitmap_last_byte =
		block_bitmap_bit_len / 8; // where the last byte begin
	uint8_t block_bitmap_last_bit =
		block_bitmap_bit_len % 8; // the remain bits in last byte
			      ...
			      char *path, void *io_buf)
{
	...
	if (parent_dir_inode->i_sectors[DIRECT_PTRS]) {
		ide_read(cur_part->my_disk,
			 parent_dir_inode->i_sectors[DIRECT_PTRS],
	...
	for (blk_i = 0; blk_i < block_cnt; blk_i++) {
		if (all_blocks[blk_i]) {
			ide_read(cur_part->my_disk, all_blocks[blk_i], io_buf,
				 1);
			for (uint8_t dir_e_i = 0; dir_e_i < dir_entrys_per_sec;
			     dir_e_i++) {
				if ((dir_e + dir_e_i)->i_no == c_inode_nr) {
					strcat(path, "/");
					strcat(path,
					       (dir_e + dir_e_i)->filename);
					return 0;
				}
			}
		}
...
char *sys_getcwd(char *buf, uint32_t size)
{
	...
	while (child_inode_nr) {
		parent_inode_nr =
			get_parent_dir_inode_nr(child_inode_nr, io_buf);
...
void fs_init()
{
	...
	for (uint8_t chan_no = 0; chan_no < chan_cnt; chan_no++) {
		...
		for (uint8_t dev_no; dev_no < 2; dev_no++) {
			...
			for (uint8_t part_i = 0; part_i < 12;
			     part_i++, part++) {
				...
				if (part->sec_cnt != 0) {
					...
					if (sb_buf->magic == 0x19491001) {
						...
						printk("formatting %s`s partition %s......\n",
						       hd->name, part->name);
						partition_format(
							part); // install the format what we know on it
					}
				}
			} // part
		} // disk
