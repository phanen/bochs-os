...
int32_t file_write(struct file *file, const void *buf, uint32_t count)
{
	...
	if (add_blocks == 0) {
		...
		if (file_will_use_blocks <= 12) {
			...
			for (blk_i = file_has_used_blocks;
			     blk_i < file_will_use_blocks; blk_i++) {
				block_lba = block_bitmap_alloc(cur_part);
				if (block_lba == -1) {
				...
				ASSERT(file->fd_inode->i_sectors[blk_i] == 0);
				file->fd_inode->i_sectors[blk_i] =
					all_blocks[blk_i] = block_lba;
				block_bitmap_idx = block_lba -
		...
		// need alloc new indirect table block first
			...
			// sync indirect table
			ide_write(cur_part->my_disk, indirect_block_table,
				  all_blocks + 12, 1);
		}
	...
	while (bytes_written < count) {
		...
		chunk_size = size_left < sec_left_bytes ? size_left :
							  sec_left_bytes;
