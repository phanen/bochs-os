...
static void release_prog_resource(struct task_struct *release_thread)
{
	...
	uint32_t bitmap_pg_cnt =
		(release_thread->userprog_vaddr.vaddr_bitmap.btmp_bytes_len) /
		PG_SIZE;
	...
	for (uint8_t fd_i = 3; fd_i < MAX_FILES_OPEN_PER_PROC; fd_i++) {
		...
		if (is_pipe(fd_i)) {
			...
			if (--file_table[global_fd].fd_pos == 0) {
				mfree_page(PF_KERNEL,
					   file_table[global_fd].fd_inode, 1);
...
pid_t sys_wait(int32_t *status)
{
	...
	while (1) {
		struct list_elem *child_elem = list_traversal(
			&thread_all_list, find_hanging_child, pthread->pid);
		...
		child_elem = list_traversal(&thread_all_list, find_child,
					    pthread->pid);
