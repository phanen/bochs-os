...
#define GET_EFLAGS(EFLAG_VAR)          \
	asm volatile("pushfl; popl %0" \
		     : "=g"(EFLAG_VAR)) // dump eflags to a macro?
...
static void exception_init(void)
{
	...
	for (i = 0; i < IDT_DESC_CNT; i++) {
		idt_table[i] =
			general_intr_handler; // init from template handler for now, we'll extend it by `register` in future
		intr_name[i] =
			"unknown intr or handler unimplemented"; // we will implement 20 of 33 entries next, so randomly name all first...
	}
