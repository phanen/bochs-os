...
static void *vaddr_get(enum pool_flags pf, uint32_t pg_cnt)
{
	...
	if (pf == PF_KERNEL) {
		...
		// bit id to mem vaddr
		vaddr_start =
			kernel_vaddr.vaddr_start + bit_idx_start * PG_SIZE;
	} else {
		struct task_struct *cur = running_thread();
		bit_idx_start =
			bitmap_scan(&cur->userprog_vaddr.vaddr_bitmap, pg_cnt);
		if (bit_idx_start == -1) {
			return NULL;
		}
		...
		vaddr_start = cur->userprog_vaddr.vaddr_start +
...
static void page_table_add(void *_vaddr, void *_page_phyaddr)
{
	uint32_t vaddr = (uint32_t)_vaddr,
		 page_phyaddr = (uint32_t)_page_phyaddr;
	...
	if (*pde & 0x00000001) {
		...
		*pde = (pde_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
		...
		pte = pte_ptr(
			vaddr); // update, now pte is a vaddr point into new `pde_phyaddr`

		memset((void *)((int)pte & 0xfffff000), 0,
		       PG_SIZE); // ensure the new alloc PT is clean

		ASSERT(!(*pte & 0x00000001));
		*pte = (page_phyaddr | PG_US_U | PG_RW_W |
			PG_P_1); // US=1,RW=1,P=1
	}
}
...
void *malloc_page(enum pool_flags pf, uint32_t pg_cnt)
{
	...
	ASSERT(pg_cnt > 0 &&
	       pg_cnt <
		       3840); // 15M/4K = 3840 page (TODO: 3840 can be dynamic...)
	...
	while (cnt--) {
		...
		if (page_phyaddr ==
		    NULL) { // TODO: better handle, need roll back all alloced vaddr and paddr
			return NULL;
		}
...
static void vaddr_remove(enum pool_flags pf, void *_vaddr, uint32_t pg_cnt)
{
	...
	if (pf == PF_KERNEL) {
		...
		bit_idx_start =
			(vaddr - cur_thread->userprog_vaddr.vaddr_start) /
			PG_SIZE;
		while (cnt < pg_cnt) {
...
static void mem_pool_init(uint32_t all_mem)
{
	...
	uint16_t all_free_pages =
		free_mem / PG_SIZE; // not devidable -> ignore it
...
void *sys_malloc(uint32_t size)
{
	...
	// small block -> adaptive
		...
		if (list_empty(&descs[desc_i].free_list)) {
			...
			for (block_i = 0;
			     block_i < descs[desc_i].blocks_per_arena;
			     block_i++) {
	...
	if (ptr != NULL) {
		...
		// small blk
			...
			if (++a->cnt == a->desc->blocks_per_arena) {
				uint32_t block_i;
				for (block_i = 0;
				     block_i < a->desc->blocks_per_arena;
				     block_i++) {
					struct mem_block *b =
						arena2block(a, block_i);
					ASSERT(elem_find(&a->desc->free_list,
							 &b->free_elem));
...
void mem_init()
{
	...
	uint32_t mem_bytes_total =
		(*(uint32_t *)(0xb00)); // from total_mem_bytes
